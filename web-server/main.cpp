...
Adafruit_HTU31D htd; //HTU31D connect to ESP32 I2C (GPIO 21 = SDA, GPIO 22 = SCL)
...
void inithtd(){
  if (!htd.begin(0x76)) {
    Serial.println("Could not find a valid HTU31D sensor, check wiring!");
    while (1);
  }
}
...
String getSensorReadings(){
  readings["temperature"] = htd.getTemperatureSensor();
  readings["humidity"] = htd.getHumiditySensor();
